...
- Swaps both policy name indexes onto ``name_cksum``:
  ...
  ``ix_policies_default_name`` (where ``scope = 'default'``, added by
  the preceding scope migration) -> ``ix_policies_default_name_cksum``.
...
This migration runs after all database FK constraints were removed
(``p1a2b3c4d5e6``), but the SQLite ``batch_alter_table`` rebuilds below
run in a combined downgrade walk alongside migrations that restore the
``conversations.agent_id`` CASCADE FK, so FK enforcement is disabled
around the batch ops — matching the convention of the adjacent
migrations.
"""
...
def upgrade() -> None:
    ...
    # the default-name partial unique index (added by the scope
    # migration). The default-name index runs on the bare op; the
    # composite-constraint swap and NOT NULL flip need batch mode.
