"""Tests for the enums-varchar→SMALLINT migration (``r1a2b3c4d5e6``).
...
def _seed_legacy_rows(engine: Engine) -> None:
    ...
    with engine.begin() as conn:
        ...
        conn.execute(
            sa.text(
                # policies.scope (NOT NULL) was added by the q1 migration that
                # precedes this one; supply it explicitly.
                "INSERT INTO policies "
                "(id, name, created_at, type, handler, enabled, scope) VALUES "
                "('p1', 'n1', 1, 'python', 'h', 1, 'default'), "
                "('p2', 'n2', 1, 'url', 'http', 1, 'default')"
            )
        )
        conn.execute(
            sa.text(
